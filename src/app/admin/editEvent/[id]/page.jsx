...
export default function EditEvent({ params }) {
  ...
  const id = use(params).id; // Korrekte Verwendung von params
  ...
  // Kategorien und Preisklassen laden
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        ...
        setIsHighlight(data.is_highlight);
        setIsHomepage(data.is_homepage);
        setSelectedCategory(data.eventcategories_id);
        setImageUrl(data.image_url);

        const formattedPriceClasses = data.event_price.map((price) => ({
          id: price.id,
          priceClassId: price.priceclass_id,
          price: price.price,
          availableTickets: price.available_tickets,
        }));
        setSelectedPriceClasses(formattedPriceClasses);
      } catch (error) {
  ...
  // Preisklassen Funktionen
  const addPriceClassDropdown = () => {
    setSelectedPriceClasses((prev) => [
      ...prev,
      { id: Date.now(), priceClassId: "", price: "", availableTickets: "" },
    ]);
  };
